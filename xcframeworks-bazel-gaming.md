# Building XCFrameworks for Gaming Applications

**Scaling iOS Modules with Bazel: rules_swift, Platform Patterns, and Cross-Platform CI**

Modern mobile games ship on iOS, Android, macOS, Windows, and web—often from a single codebase. When third-party SDKs only support a subset of platforms (like iOS-only Facebook SDK), you need sophisticated build patterns to maintain a unified dependency graph. This guide covers XCFramework integration, platform-conditional compilation, and CI strategies for large-scale gaming projects.
